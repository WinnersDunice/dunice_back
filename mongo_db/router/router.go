...
func Rout(client *mongo.Client) error {
    ...
    svgCollection := client.Database("mongo").Collection("svgImages")
    ...
    // SVG Images routes
    v6 := chi.NewRouter()
    api.Mount("/svgImages", v6)

    // Create a new SVG image
    v6.Post("/", func(w http.ResponseWriter, r *http.Request) {
        w.Header().Set("Access-Control-Allow-Origin", "*")
        log.Println("Handling POST /svgImages request")
        svgImage := new(fn.SVGImage)
        ...
        // You can also read additional parameters from the request if needed
        svgImage.Name = r.URL.Query().Get("name")
        svgImage.OfficeID = r.URL.Query().Get("officeId")

        if err := fn.CreateSVGImage(svgCollection, svgImage); err != nil {
            log.Printf("Error creating SVG image: %v", err)
            http.Error(w, err.Error(), http.StatusInternalServerError)
            return
        }
        log.Printf("SVG image created: %v", svgImage)
        json.NewEncoder(w).Encode(svgImage)
    })

    // Get SVG image by ID
    v6.Get("/{id}", func(w http.ResponseWriter, r *http.Request) {
        id := chi.URLParam(r, "id")
        log.Printf("Handling GET /svgImages/%s request", id)
        svgImage, err := fn.GetSVGImageByID(svgCollection, id)
        if err != nil {
            log.Printf("Error getting SVG image by ID: %v", err)
            http.Error(w, err.Error(), http.StatusInternalServerError)
            return
        }
        json.NewEncoder(w).Encode(svgImage)
